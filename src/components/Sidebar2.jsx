...
import { 
    LayoutDashboard, GraduationCap, Users, BookOpen, ChevronRight,
    Building, User, ClipboardList, Info
} from 'lucide-react';
...
const Sidebar2 = () => {
    ...
    useEffect(() => {
        ...
        const newOpenMenus = {};
        if (path.startsWith('/siswa') || path.startsWith('/orangtua')) newOpenMenus.siswa = true;
        if (path.startsWith('/pegawai') || path.startsWith('/jabatan')) newOpenMenus.pegawai = true;
    ...
    return (
        ...
        <aside className="fixed top-0 left-0 w-72 h-full bg-gradient-to-b from-[#111827] to-[#1a254a] flex flex-col z-40 border-r border-white/10">
            ...
            <div className="flex items-center gap-4 px-6 py-5 border-b border-white/10">
                ...
                />
                <div>
                    <Link to="/" className="text-xl font-bold text-white tracking-wider">GuestBook</Link>
            ...
            <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                <motion.ul variants={listVariants} initial="hidden" animate="visible">
                    ...
                    <motion.li variants={itemVariants} className="space-y-1">
                        <DropdownButton onToggle={() => handleMenuToggle('siswa')} isOpen={openMenus.siswa} icon={GraduationCap}>Siswa</DropdownButton>
                        <AnimatePresence>
                            {openMenus.siswa && (
                                <motion.div variants={dropdownVariants} initial="hidden" animate="visible" exit="hidden" className="overflow-hidden">
                                    <NavItem to="/siswa" icon={User} isSubItem>Data Siswa</NavItem>
                                    <NavItem to="/orangtua" icon={Users} isSubItem>Data Orang Tua</NavItem>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.li>
                    ...
                    {/* Dropdown Pegawai */}
                    <motion.li variants={itemVariants} className="space-y-1">
                        <DropdownButton onToggle={() => handleMenuToggle('pegawai')} isOpen={openMenus.pegawai} icon={Building}>Pegawai</DropdownButton>
                        <AnimatePresence>
                            {openMenus.pegawai && (
                                <motion.div variants={dropdownVariants} initial="hidden" animate="visible" exit="hidden" className="overflow-hidden">
                                    <NavItem to="/jabatan" icon={ClipboardList} isSubItem>Data Jabatan</NavItem>
                                    <NavItem to="/pegawai" icon={Users} isSubItem>Data Pegawai</NavItem>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.li>
                    ...
                    {/* Dropdown Buku Tamu */}
                    <motion.li variants={itemVariants} className="space-y-1">
                        <DropdownButton onToggle={() => handleMenuToggle('bukuTamu')} isOpen={openMenus.bukuTamu} icon={BookOpen}>Buku Tamu</DropdownButton>
                        <AnimatePresence>
                            {openMenus.bukuTamu && (
                                <motion.div variants={dropdownVariants} initial="hidden" animate="visible" exit="hidden" className="overflow-hidden">
                                    <NavItem to="/bukutamu" icon={BookOpen} isSubItem>Data Buku Tamu</NavItem>
                                </motion.div>
                            )}
                        </AnimatePresence>
                    </motion.li>
