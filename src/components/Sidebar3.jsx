    ...
    return (
        ...
        <aside className="fixed top-0 left-0 w-72 h-full bg-gradient-to-br from-[#101831] to-[#1a254a] flex flex-col z-40 border-r border-white/10">
            ...
            <nav className="flex-1 p-4 space-y-2 overflow-y-auto">
                ...
                {/* Dropdown Menu */}
                {[
                  { key: 'siswa', icon: GraduationCap, label: 'Siswa', sub: [{path: '/siswa', label: 'Data Siswa', icon: User}, {path: '/orangtua', label: 'Data Orang Tua', icon: Users}] },
                  { key: 'pegawai', icon: Building, label: 'Pegawai', sub: [{path: '/jabatan', label: 'Data Jabatan', icon: ClipboardList}, {path: '/pegawai', label: 'Data Pegawai', icon: Users}] },
                  { key: 'bukuTamu', icon: BookOpen, label: 'Buku Tamu', sub: [{path: '/bukutamu', label: 'Data Buku Tamu', icon: BookOpen}] },
                ].map(menu => (
                  <div key={menu.key} className="space-y-1">
                      <button onClick={() => handleMenuToggle(menu.key)} className={getLinkClass({})}>
                          <div className="flex items-center gap-4">
                              <menu.icon size={20} /> <span>{menu.label}</span>
                          </div>
                          <ChevronRight size={16} className={`transition-transform duration-300 ${openMenus[menu.key] ? 'rotate-90' : ''}`} />
                      </button>
                      <AnimatePresence>
                          {openMenus[menu.key] && (
                              <motion.div variants={dropdownVariants} initial="hidden" animate="visible" exit="hidden" className="overflow-hidden">
                                  ...
                                  <div className="pl-8 ml-4 border-l border-gray-700 space-y-1 pt-1">
                                    {menu.sub.map(subItem => (
                                        <NavLink key={subItem.path} to={subItem.path} className={getSubLinkClass}>
