...
const Jabatan = () => {
    ...
    const fetchJabatan = () => {
        setLoading(true);
        setTimeout(() => {
            const dataWithNo = dummyJabatan.map((item, index) => ({ ...item, no: page * rowsPerPage + index + 1 }));
            setJabatan(dataWithNo);
            setLoading(false);
        }, 1500);
    };

    useEffect(() => { fetchJabatan() }, []);
    ...
    const handleFormSubmit = (e, actionType) => {
        ...
        setTimeout(() => {
            ...
            console.log(`${actionType} data jabatan:`, formData);
            closeModal();
            setIsSubmitting(false);
            showNotif('success', `Data jabatan berhasil di${actionType === 'Tambah' ? 'tambahkan' : 'perbarui'}!`);
            fetchJabatan(); // Refresh data
        }, 1000);
    };
    ...
    const confirmDelete = () => {
        setIsSubmitting(true);
        setTimeout(() => {
            console.log("Hapus jabatan:", modalState.data);
            closeModal();
            setIsSubmitting(false);
            showNotif('success', `Data ${modalState.data.nama_jabatan} berhasil dihapus.`);
            fetchJabatan(); // Refresh data
        }, 1000);
    };
    ...
    <>
        ...
        >
            <div className="mb-6 border-b pb-4">
                <h1 className="text-3xl font-bold text-gray-800">Manajemen Data Jabatan</h1>
                <p className="text-gray-500 mt-1">Kelola daftar jabatan di halaman ini.</p>
            </div>
                    ...
                    <table className="min-w-full w-full table-auto border-collapse">
                        ...
                        {paginatedItems.map((j, index) => (
                            <tr key={j.id} className="hover:bg-gray-50 text-gray-700">
                                ...
                                <td className="px-3 py-3 whitespace-nowrap font-medium text-gray-900 text-center">{j.nama_jabatan}</td>
                                <td className="px-3 py-3 whitespace-nowrap text-center space-x-2">
                                    <button onClick={() => setModalState({ type: 'edit', data: j })}
                                        className="bg-amber-100 text-amber-800 font-semibold p-2 rounded-lg hover:bg-amber-200 transition">
                                        <Edit className="w-4 h-4" />
                                    </button>
                                    <button onClick={() => setModalState({ type: 'delete', data: j })}
        ...
        {/* --- MODAL TAMBAH DATA --- */}
        <Modal isOpen={modalState.type === 'add'} onClose={closeModal} title="Tambah Jabatan Baru">
        ...
        {/* --- MODAL EDIT DATA --- */}
        <Modal isOpen={modalState.type === 'edit'} onClose={closeModal} title={`Edit Jabatan: ${modalState.data?.nama_jabatan}`}>
            {modalState.data && (
            ...
            {modalState.data && (
                <div className="text-center">
                    ...
                    <p className="text-gray-600 text-lg mb-6">
                        ...
                        <br />
                        <span className="font-bold text-gray-900">{modalState.data.nama_jabatan}</span>?
                    </p>
