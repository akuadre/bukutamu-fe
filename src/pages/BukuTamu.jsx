        ...
        <table className="min-w-full w-full table-auto animate-pulse">
            <thead className="bg-gray-800 text-white text-center">
                <tr>
                    ...
                    <th className="px-3 py-3 w-24 border-[0.5px] border-gray-600 text-xs font-medium uppercase tracking-wider">Aksi</th>
                </tr>
            </thead>
            <tbody>
                {[...Array(rowCount)].map((_, index) => (
                    <tr key={index} className="border-b border-gray-200">
                        ...
                        <td className="px-3 py-3"><div className="h-4 bg-gray-200 rounded w-3/4"></div></td>
                        <td className="px-3 py-3"><div className="h-4 bg-gray-200 rounded"></div></td>
                        <td className="px-3 py-3"><div className="h-4 bg-gray-200 rounded w-3/4"></div></td>
                        <td className="px-3 py-3"><div className="h-4 bg-gray-200 rounded w-2/3"></div></td>
                        <td className="px-3 py-3"><div className="h-4 bg-gray-200 rounded w-5/6"></div></td>
                        <td className="px-3 py-3"><div className="w-16 h-16 bg-gray-200 rounded-md mx-auto"></div></td>
...
const BukuTamu = () => {
    ...
    const fetchBukuTamu = () => {
        setLoading(true);
        setTimeout(() => {
            const dataWithNo = dummyBukuTamu.map((item, index) => ({ ...item, no: page * rowsPerPage + index + 1 }));
            setBukuTamu(dataWithNo);
            setLoading(false);
        }, 1500);
    };

    useEffect(() => { fetchBukuTamu() }, []);
    ...
    <>
        ...
        >
            ...
            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                <div className="relative w-full md:w-80">
                    ...
                    <input type="text" placeholder="Cari tamu..." value={filterText} onChange={(e) => setFilterText(e.target.value)}
            ...
            {loading ? <LoadingTable rowCount={rowsPerPage} /> : (
                    ...
                    <table className="min-w-full w-full table-auto border-collapse">
                        ...
                        {paginatedItems.map((tamu, index) => (
                            <tr key={tamu.id} className="hover:bg-gray-50 text-gray-700">
                                ...
                                <td className="px-3 py-3 whitespace-nowrap text-left">{tamu.siswa?.namasiswa || '-'}</td>
                                <td className="px-3 py-3 whitespace-nowrap text-left">{tamu.pegawai?.nama_pegawai} ({tamu.jabatan?.nama_jabatan})</td>
                                ...
                                <td className="px-3 py-3 whitespace-nowrap text-center">
                                    ...
                                    <button onClick={() => setModalState({ type: 'detail', data: tamu })}
        ...
        {/* --- MODAL DETAIL DATA --- */}
        <Modal isOpen={modalState.type === 'detail'} onClose={closeModal} title={`Detail Kunjungan: ${modalState.data?.nama}`}>
            {modalState.data && (
                <div className="space-y-6">
                    <div className="flex flex-col sm:flex-row items-center gap-6 pb-6 border-b">
                        {modalState.data.foto_tamu ? (
                            <img src={`https://placehold.co/100x100/E0E7FF/4338CA?text=Foto`} alt="Foto Tamu" className="w-24 h-24 object-cover rounded-full shadow-md"/>
                        ) : (
                        ...
                        <div className="text-center sm:text-left">
                            <h3 className="text-2xl font-bold text-gray-900">{modalState.data.nama}</h3>
                            <p className="text-gray-500 capitalize">{modalState.data.role === 'ortu' ? `Orang Tua dari ${modalState.data.siswa?.namasiswa}` : modalState.data.instansi}</p>
                        </div>
                    </div>
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4">
                        <div className="flex flex-col gap-2">
                          ...
                          <a 
                            href={`https://wa.me/${formatPhoneNumber(modalState.data.kontak)}`}
                            target="_blank"
                        ...
                        <DetailItem label="Bertemu Dengan" value={`${modalState.data.pegawai?.nama_pegawai} (${modalState.data.jabatan?.nama_jabatan})`} />
                        <DetailItem label="Tanggal Kunjungan" value={formatDate(modalState.data.created_at)} />
                        <DetailItem label="Keperluan" value={modalState.data.keperluan} fullWidth={true} />
                    </div>
                    <div className="flex justify-end pt-4">
                        <button onClick={closeModal} className="px-6 py-2 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 transition">
                            Tutup
                        </button>
                    </div>
                </div>
