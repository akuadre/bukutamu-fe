...
// --- DUMMY DATA ---
const dummyPegawai = [
    { id: 1, nama_pegawai: "Endang S.", jenis_kelamin: "P", jabatan: { id: 1, nama_jabatan: "Kepala Sekolah" }, agama: { id: 1, agama: "Islam" }, kontak: "081234567890" },
    { id: 2, nama_pegawai: "Joko Widodo", jenis_kelamin: "L", jabatan: { id: 3, nama_jabatan: "Guru BP" }, agama: { id: 1, agama: "Islam" }, kontak: "081234567891" },
    { id: 3, nama_pegawai: "Sri Mulyani", jenis_kelamin: "P", jabatan: { id: 4, nama_jabatan: "Wali Kelas IX A" }, agama: { id: 2, agama: "Kristen" }, kontak: "081234567892" },
    { id: 4, nama_pegawai: "Bambang Soesatyo", jenis_kelamin: "L", jabatan: { id: 6, nama_jabatan: "Guru Matematika" }, agama: { id: 3, agama: "Katolik" }, kontak: "081234567893" },
    { id: 5, nama_pegawai: "Puan Maharani", jenis_kelamin: "P", jabatan: { id: 7, nama_jabatan: "Guru Bahasa Indonesia" }, agama: { id: 4, agama: "Hindu" }, kontak: "081234567894" },
    { id: 6, nama_pegawai: "Prabowo Subianto", jenis_kelamin: "L", jabatan: { id: 8, nama_jabatan: "Penjaga Sekolah" }, agama: { id: 1, agama: "Islam" }, kontak: "081234567895" }
];
// --- END DUMMY DATA ---
...
const Pegawai = () => {
    ...
    const fetchPegawai = () => {
        setLoading(true);
        setTimeout(() => {
            const dataWithNo = dummyPegawai.map((item, index) => ({ ...item, no: page * rowsPerPage + index + 1 }));
    ...
    const handleFormSubmit = (e, actionType) => {
        ...
        setTimeout(() => {
            ...
            console.log(`${actionType} data pegawai:`, formData);
            closeModal();
            setIsSubmitting(false);
            showNotif('success', `Data pegawai berhasil di${actionType === 'Tambah' ? 'tambahkan' : 'perbarui'}!`);
            fetchPegawai();
        }, 1000);
    };
    ...
    const confirmDelete = () => {
        ...
        setTimeout(() => {
            ...
            closeModal();
            setIsSubmitting(false);
            showNotif('success', `Data ${modalState.data.nama_pegawai} berhasil dihapus.`);
            fetchPegawai();
        }, 1000);
    };
    ...
    <>
        ...
        >
            <div className="mb-6 border-b pb-4">
                ...
                <p className="text-gray-500 mt-1">Kelola daftar pegawai di halaman ini.</p>
            </div>

            <div className="flex flex-col md:flex-row items-center justify-between gap-4 mb-6">
                <div className="relative w-full md:w-80">
                    <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 w-5 h-5 text-gray-400" />
                    <input type="text" placeholder="Cari pegawai..." value={filterText} onChange={(e) => setFilterText(e.target.value)}
        ...
        {/* --- MODAL TAMBAH DATA --- */}
        <Modal isOpen={modalState.type === 'add'} onClose={closeModal} title="Tambah Data Pegawai Baru">
            <form onSubmit={(e) => handleFormSubmit(e, 'Tambah')} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                <div className="md:col-span-2">
                    <FormInput label="Nama Pegawai" id="nama_pegawai" name="nama_pegawai" placeholder="Masukkan nama lengkap" required />
                </div>
                <FormInput label="Jabatan" id="jabatan" name="jabatan" placeholder="Contoh: Staf Pengajar" required />
        ...
        {/* --- MODAL EDIT DATA --- */}
        <Modal isOpen={modalState.type === 'edit'} onClose={closeModal} title={`Edit Data: ${modalState.data?.nama_pegawai}`}>
            {modalState.data && (
                <form onSubmit={(e) => handleFormSubmit(e, 'Edit')} className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
                    <div className="md:col-span-2">
                        <FormInput label="Nama Pegawai" id="nama_pegawai_edit" name="nama_pegawai" defaultValue={modalState.data.nama_pegawai} required />
                    </div>
                     {/* Sebaiknya Jabatan & Agama menggunakan dropdown/select */}
                    <FormInput label="Jabatan" id="jabatan_edit" name="jabatan" defaultValue={modalState.data.jabatan?.nama_jabatan} required />
            ...
            {modalState.data && (
                <div className="text-center">
                    ...
                    <p className="text-gray-600 text-lg mb-6">
                        ...
                        <br />
                        <span className="font-bold text-gray-900">{modalState.data.nama_pegawai}</span>?
                    </p>
