...
const OrangTua = () => {
  ...
  const [ortuToDelete, setOrtuToDelete] = useState(null);
  ...
  const fetchOrangTua = () => {
    setLoading(true);
    setTimeout(() => {
        const dataWithNo = dummyOrangTua.map((item, index) => ({
            ...item,
            no: index + 1,
        }));
  ...
  const handleDelete = (ortu) => {
    setOrtuToDelete(ortu);
    setShowDeleteConfirmModal(true);
  };

  const confirmDelete = () => {
    console.log("Hapus orang tua:", ortuToDelete);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        ...
        <button
          onClick={() => alert("Navigate to Tambah Orang Tua page")}
        ...
        <div className="overflow-x-auto text-sm">
          <table className="min-w-full w-full table-auto border-collapse">
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {paginatedItems.map((ortu) => (
                <tr key={ortu.id} className="hover:bg-gray-50 text-gray-700">
                  <td className="px-3 py-3 whitespace-nowrap text-center">{ortu.no}</td>
                  ...
                  <td className="px-3 py-3 whitespace-nowrap text-center space-x-2">
                    <button onClick={() => alert(`Edit ortu: ${ortu.nama_ortu}`)} className="bg-yellow-100 text-yellow-800 font-semibold p-2 rounded-lg hover:bg-yellow-200 transition">
                        <Edit className="w-4 h-4" />
                    </button>
                    <button onClick={() => handleDelete(ortu)} className="bg-red-100 text-red-800 font-semibold p-2 rounded-lg hover:bg-red-200 transition">
      ...
      {showDeleteConfirmModal && ortuToDelete && (
        <div className="fixed inset-0 flex items-center justify-center bg-black/70 z-50 p-4">
          <div className="bg-white p-8 rounded-xl shadow-2xl w-full max-w-sm text-center">
            <h2 className="text-2xl font-bold text-gray-800 mb-4">Konfirmasi Hapus</h2>
            <p className="text-gray-600 mb-6">Apakah Anda yakin ingin menghapus data orang tua <strong className="text-gray-900">{ortuToDelete.nama_ortu}</strong>?</p>
