...
const StatCard = ({ title, value, icon, color, link }) => (
  <div className={`bg-gradient-to-br ${color} rounded-xl shadow-lg text-white overflow-hidden transform hover:-translate-y-1 transition-transform duration-300`}>
    ...
    <Link to={link} className="block bg-black bg-opacity-20 hover:bg-opacity-30 text-white text-center py-2.5 px-4 transition-colors">
      <span className="flex items-center justify-center text-sm">Lihat Detail <ArrowRight className="w-4 h-4 ml-1.5" /></span>
    </Link>
  </div>
);

// Komponen Skeleton untuk Kartu
const StatCardSkeleton = () => <div className="bg-gray-200 rounded-xl shadow-lg h-36 animate-pulse"></div>;
...
const Dashboard = () => {
  ...
  const [selectedWeek, setSelectedWeek] = useState({ year: 2025, month: 8, week: 4 });
  ...
  const handleSync = async () => {
    setIsSyncing(true);
    try {
      const response = await axios.post(`${API_URL}/sync-manual`);
      alert(response.data.message); // Tampilkan pesan sukses
      fetchDashboard(); // Panggil ulang fetchDashboard untuk me-refresh data
    } catch (error) {
      console.error("Gagal memulai sinkronisasi:", error);
      alert("Gagal memulai sinkronisasi. Cek console untuk detail.");
    } finally {
      setIsSyncing(false);
    }
  };
  ...
  const chartOptions = { responsive: true, maintainAspectRatio: false, plugins: { legend: { display: false } }, scales: { x: { grid: { display: false } }, y: { beginAtZero: true, ticks: { stepSize: 5 } } } };
  ...
  const getChartTitle = () => {
      switch (filterType) {
          case "harian":
              return `Statistik Kunjungan - ${new Date(selectedDate + "T00:00:00").toLocaleDateString("id-ID", { day: "numeric", month: "long", year: "numeric" })}`;
          case "mingguan":
              return `Statistik Kunjungan - Minggu ke-${selectedWeek.week}, ${new Date(selectedWeek.year, selectedWeek.month).toLocaleString("id-ID", { month: "long", year: "numeric" })}`;
          case "bulanan":
              return `Statistik Kunjungan - ${new Date(selectedMonth.year, selectedMonth.month).toLocaleString("id-ID", { month: "long", year: "numeric" })}`;
          case "tahunan":
    ...
    <div className="space-y-8">
      <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4">
        ...
        <motion.div initial={{ opacity: 0, scale: 0.8 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 0.5, delay: 0.2 }}>
            <button onClick={handleSync} disabled={isSyncing} className={`flex items-center justify-center gap-2 px-4 py-2 font-semibold text-white rounded-lg shadow-md transition-colors duration-300 ${ isSyncing ? 'bg-gray-500 cursor-not-allowed' : 'bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-blue-500 focus:ring-offset-2' }`}>
                {isSyncing ? ( <> <Loader className="w-5 h-5 animate-spin" /> <span>Menyinkronkan...</span> </> ) : ( <> <RefreshCw className="w-5 h-5" /> <span>Sinkronkan Data</span> </> )}
            </button>
        </motion.div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        <motion.div variants={itemVariants} className="bg-white shadow-lg rounded-xl p-6 lg:col-span-2">
           <div className="flex flex-col sm:flex-row justify-between sm:items-center gap-4 mb-4">
              <h2 className="text-xl font-bold text-gray-800 flex items-center"><BarChart2 className="w-6 h-6 mr-2 text-blue-500" />{getChartTitle()}</h2>
